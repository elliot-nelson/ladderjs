...
export const Sprite = {
    ...
    init() {
        ...
        Sprite.hud_crosshair = SpriteSheet.arrow.map(initBasicSprite);

        // Pages
        Sprite.page = SpriteSheet.page.map(initBasicSprite);

        // Tiles
        Sprite.tiles = SpriteSheet.tiles2.map(data => initBasicSprite(data, { x: 8, y: 17 }));
...
// Fill a given image with the specified color (ignoring transparent pixels)
function fillOpaquePixels(source, color) {
    let canvas = createCanvas(source.width, source.height);
    canvas.ctx.drawImage(source, 0, 0);
    canvas.ctx.globalCompositeOperation = 'source-atop';
